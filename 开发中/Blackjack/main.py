...
若需要修改插件NameSpace，请使用VSCode或其他软件全局替换将Blackjack替换为你的插件名称。
'''
...
class Event(object):
    def init(plugin_event, Proc):
        ...
        Blackjack.msgReply.unity_init(plugin_event, Proc)

    def init_after(plugin_event, Proc):
        # 在所有插件初始化完成后调用
        # 这里可以进行一些依赖于其他插件的初始化工作
        Blackjack.msgReply.data_init(plugin_event, Proc)

    def private_message(plugin_event, Proc):
        # 私聊消息处理
        Blackjack.msgReply.unity_reply(plugin_event, Proc)
    ...
    def menu(plugin_event, Proc):
        ...
        #         pass
        #     elif plugin_event.data.event == 'Blackjack_Menu_002':
        #         pass
        pass