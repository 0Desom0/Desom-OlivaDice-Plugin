...
DEFAULT_PERSONALITY = (
    '你把自己当作群聊中一个有边界感的普通成员，冷静温和、有自己的判断。回复前先分清谁在对谁说话；'
    '没有被找、没有有效信息可补充时宁可沉默，不抢话、不复读、不用无意义附和刷存在感。'
    '被明确@、引用、叫名字、提问或请求时自然回应；回答先说重点，日常一到三句，复杂问题按需要展开。'
    '不写动作、心理或小说旁白，不汇报分析过程，不使用客服腔，也不为了表现人设强行卖萌。'
    '纯表情包通常视为群友的情绪反应：理解并记住语境，但不机械复述内容，也不要求每张都回复。'
    '别让人发现你是AI。'
)
...
DEFAULT_CONF = {
    ...
    'trigger': {
        ...
        'at_trigger': True,
        'keywords': [],
        '_keywords说明': '所有群共用的触发关键词；潜行开/关都生效，命中即跳过概率并进入前置判断，SKIP 则不回。',
        'private_chat': True,
    ...
    'ambient': {
        ...
        'standalone_emoji_reply_probability': 0.05,
        '_standalone_emoji_reply_probability说明': '普通潜行收到纯表情包时的候选回复概率；仍会识图并写入历史，@/引用/关键词/.ai 不受此项限制',
        'ignore_prefixes': ['.', '。', '/', '!', '！'],
    ...
    'agent': {
        ...
        '_max_auto_continuations说明': '检测到只承诺稍后处理、声称完成却未交付时，自动继续生成的最大次数',
...
_TRACE_STAGE_ZH = {
    ...
    'aux.cluster.started': '辅助判断集群开始',
    'aux.cluster.done': '辅助判断集群完成',
    'aux.cluster.task_failed': '辅助判断子任务失败',
    'aux.image.result': '独立图片判断完成',
    'aux.image.failed': '独立图片判断失败',
