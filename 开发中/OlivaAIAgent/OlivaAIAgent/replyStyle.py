...
_INTERNAL_DELIBERATION_PATTERNS = (
    ...
    re.compile(r'用户[“"\']?.{0,40}(?:要求我|让我|要我).{0,50}(?:设计|回复|回答|输出|构思|创作)'),
    re.compile(r'(?:当前发言者|本轮发言者|发言者身份)\s*(?:是|为|身份(?:是|为)?|列为)'),
    re.compile(r'(?:当前|最新)(?:一条)?消息.{0,30}(?:来自|是|内容(?:是|为)?|发了)'),
    re.compile(r'(?:让我|我需要|我来)(?:先)?(?:构思|组织|数一下|数一数|分析|判断|检查|梳理)'),
    re.compile(r'(?:只有|仅有|现在只有).{0,12}(?:这一条|一个)(?:有效)?任务'),
    re.compile(r'(?:最后|然后|接着).{0,30}(?:按格式输出|输出\s*JSON|JSON\s*格式)'),
