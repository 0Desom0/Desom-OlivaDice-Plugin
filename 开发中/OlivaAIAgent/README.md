...
- 前置便宜模型同时负责参与判定和工具路由；正式回复只携带本轮相关工具，路由失败会回退全量工具，不会静默丢失能力。
- 后台记忆提炼、定时提醒措辞与技能元数据翻译优先走辅助模型；未配置辅助模型时仍回退主模型。
...
插件自带回归测试当前共 **143 项**，覆盖配置迁移、群路由、Core/Logger 出站桥接、内容安全、词库更新、伪 JSON 解包、消息引用、语音、MCP、视觉和记忆等关键路径。
