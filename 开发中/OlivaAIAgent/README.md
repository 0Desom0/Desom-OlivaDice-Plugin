...
- 潜行**开启**的群：`.ai` 直接要求主模型回复；关键词/@/引用机器人跳过概率并进入小模型判定；普通闲聊按概率自行插话；同时记录群滚动历史作上下文。
...
**潜行 vs AI 模式：整合而非二选一**（v2.3）。群关键词始终可触发整合请求并跳过概率，但仍进入前置小模型；只有本群潜行开启后，@机器人和引用机器人回复才会跳过概率、进入前置小模型，`NEXT` 后再调用主模型。普通闲聊也只有潜行开启后才会进入概率门。整合请求同时带上人设、群上下文、知识、技能、视觉和 Agent 工具；开关为 `ambient.integrate_hard_trigger`（默认开）。

**统一触发项**：`trigger.prefix` / `trigger.keywords` 是所有群共用的全局值，不再提供群级覆盖。关键词不受潜行开关影响，命中即跳过概率并进入前置判断。每条消息仍按（bot|群|消息id）去重，重复投递只处理一次。
...
- 阿里云模式支持 `language_type`、`instructions` 与 `optimize_instructions`。每次 `instructions` 都由主模型在调用 `send_voice` 时根据当前上下文动态生成，必须用英文写，只描述本次语速、情绪、音量、停顿和语调，不写入配置或记忆，也不是第二套人格提示词；接口返回的临时音频 URL 会立即下载，并按 URL、Content-Type 或音频头识别真实格式。
...
- **统一触发关键词**：所有群共用 `trigger.keywords`，潜行开 / 关都用它触发（命中即跳过概率并进入前置判断），不再保留群级关键词以及 `wake_words`/`reply_keywords` 两套重复配置。
...
- `ambient.first_thinking` 处理普通潜行消息及定向触发：普通消息先通过 `reply_probability` 概率门，命中后调用前置模型；被 @、引用机器人或命中关键词会跳过概率，直接交给前置模型判断。前置模型返回 `SKIP` 时不进入主模型，返回 `NEXT` 时主模型必须回应。只有 `.ai` 跳过前置判断并直接要求主模型回应。
...
- **定向请求不丢失**：`.ai`/@/引用机器人/关键词不再被概率或节律锁的“让位”机制静默丢弃；@/引用/关键词仍可由前置小模型决定 `SKIP`。
