...
**任务交付会自动续行。** 普通 Agent 与潜行群聊都会在终止前检查回复是否只是“稍等、马上发、已经整理好”之类的进度承诺，或虚构结果已放入文件夹但没有真正交付。命中后不发送这条空承诺，而是在同一请求内继续生成或调用工具；`agent.max_auto_continuations` 控制最多续行次数，避免无限循环。
