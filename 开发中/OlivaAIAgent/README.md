...
- 潜行**开启**的群：`.ai` 直接要求主模型回复；关键词跳过小模型后由主模型二次判断是否开口；@/引用机器人跳过概率并进入小模型判定；普通闲聊按概率自行插话；同时记录群滚动历史作上下文。
...
**潜行 vs AI 模式：整合而非二选一**（v2.3）。群关键词始终可触发整合请求并直接跳过小模型，进入主模型二次判断；只有本群潜行开启后，@机器人和引用机器人回复才会跳过概率、进入前置小模型，`NEXT` 后再调用主模型。普通闲聊也只有潜行开启后才会进入概率门。整合请求同时带上人设、群上下文、知识、技能、视觉和 Agent 工具；开关为 `ambient.integrate_hard_trigger`（默认开）。

**统一触发项**：`trigger.prefix` / `trigger.keywords` 是所有群共用的全局值，不再提供群级覆盖。关键词不受潜行开关影响，命中即跳过概率和前置小模型进入主模型，主模型仍可选择沉默。每条消息仍按（bot|群|消息id）去重，重复投递只处理一次。
...
- **统一触发关键词**：所有群共用 `trigger.keywords`，潜行开 / 关都用它触发（命中即跳过概率和前置小模型，主模型仍可选择沉默），不再保留群级关键词以及 `wake_words`/`reply_keywords` 两套重复配置。
...
- `ambient.first_thinking` 处理普通潜行消息及定向触发：普通消息先通过 `reply_probability` 概率门，命中后调用前置模型；被 @ 或引用机器人消息会跳过概率，直接交给前置模型判断。前置模型返回 `SKIP` 时不进入主模型，返回 `NEXT` 时主模型必须回应。关键词跳过前置判断后由主模型二次判断是否开口；只有 `.ai` 跳过前置判断并直接要求主模型回应。
