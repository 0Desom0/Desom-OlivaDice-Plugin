...
**群级触发项**：`trigger.prefix` / `trigger.keywords` 是全局默认值；每个群可在 `groups.json` 或 GUI 中分别覆盖。字段留空表示继承，`[]` 表示该群明确禁用。关键词不受潜行开关影响，命中即强制回复。每条消息仍按（bot|群|消息id）去重，重复投递只处理一次。
...
- **群级触发关键词**：`trigger.keywords` 作为默认值，潜行开 / 关都用它触发（命中即强制回复）；需要差异化时可在群级设置中覆盖，不再保留 `wake_words`/`reply_keywords` 两套重复配置。
...
插件自带回归测试当前共 **139 项**，覆盖配置迁移、群路由、Core/Logger 出站桥接、内容安全、词库更新、伪 JSON 解包、消息引用、语音、MCP、视觉和记忆等关键路径。
...
- “群级设置”集中管理插件全局启用、白名单模式、新群默认开关、默认前缀/关键词，以及 `groups.json` 中每群的插件开关、潜行、前缀、关键词、高危工具和记忆覆盖。独立的“全局启用”和“群白名单”分类已移除。
