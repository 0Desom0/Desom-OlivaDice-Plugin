...
• .help shst - 查看人物卡录入帮助''',
    ...
    'sh': '''【狩魂者检定】
.sh(a)(h)(m)(s数字(固定值)kh/kl)(b/p数字)(x数字u/d+)(掷骰表达式)(~掷骰表达式)(#)(a)(s数字(固定值)kh/kl)(b/p数字)(x数字u/d+)(掷骰表达式)(@他人)

参数说明：
- h：暗骰
- m：移除默认 D20
- s数量(固定值)(kh/kl)：设置 D20 数量与固定值（可设多个，用逗号分隔），并用 kh（取最高）或 kl（取最低）修饰。
- b/p数字：加/减成功等级（默认1）
- x数字(u/d)：指定 1D20 固定值，可选是否向上/向下修正
- a: D20 中的 1 和 2 均视为大失败
...
2、玩家在填写完角色卡并审核通过后，将附表“角色卡”中的.st数据复制粘贴，输入群内，这样就完成了基础数据的录入。
