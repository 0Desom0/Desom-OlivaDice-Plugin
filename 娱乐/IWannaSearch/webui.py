...
def get_state(Proc, requested_bot_hash: str = '') -> dict:
    ...
    state = {
        ...
        'global_config': {
            ...
            'selection_timeout_seconds': int(global_config.get('selection_timeout_seconds', config.selection_timeout_seconds)),
            'max_download_concurrency': int(global_config.get('max_download_concurrency', config.download_concurrency_default)),
        },
    ...
    with utils.file_lock:
        ...
        if action == 'save_global':
            changes = {
                ...
                'selection_timeout_seconds': require_int(payload, 'selection_timeout_seconds', min_val=10, max_val=3600),
                'max_download_concurrency': require_int(payload, 'max_download_concurrency', min_val=config.download_concurrency_min, max_val=config.download_concurrency_max),
            }
            global_config = utils.load_global_config()
