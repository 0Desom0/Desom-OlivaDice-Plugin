...
这是一个 Magisk、KernelSU、APatch 通用模块，附带一个极简控制 App。模块不依赖 ADB、Termux、`curl`、`scp` 或 `sshpass`。

安装后打开“Lanota 国服 Token 控制台”，先点击“开始扫描”，再从国服 Lanota 打开 Portal。模块会保持一次连续的捕获进程，轮询 `/proc/net/unix` 中的 WebView/Chrome DevTools socket，并订阅 `Network.requestWillBeSent`，因此 Portal 朝浏览器发送 Token 的瞬时请求也不会被扫描间隔漏掉。同时会读取 Portal 页面的 `localStorage`，并把 Root Local Storage 文件扫描作为兜底。

一次扫描默认持续 10 分钟，直到捕获成功、用户点击停止，或超时。不是每隔几秒只扫一瞬间。DevTools 会检查所有可调试页面，但只接受严格通过国服 JWT 与 `/portal/api/me` 验证的 Token；文件兜底同步检查 Chrome、Edge 和游戏 WebView。

检测到有效 Token 后，界面会显示账号标识和过期时间，此时才可以点击“上传 Token”。扫描阶段不会连接 SSH，也不会写服务器；上传成功或失败都会直接显示在窗口中。“清除待上传”会删除手机上的待上传状态。
...
```text
dist/LanotaChinaTokenUploader-v1.2.0-configured.zip
```

同时会生成可单独安装的 `dist/LanotaControl-v1.2.0.apk`。APK 必须和 Root 模块一起使用，本身不保存 SSH 密码或 Token。
...
4. 先点“开始扫描”，保持界面显示正在监听，再从国服 Lanota 正常进入 Portal 并完成登录。
5. 出现“已检测并验证，可点击上传”后，再点“上传 Token”。
...
- 模块只连接本机可调试 WebView/浏览器的 DevTools socket，不安装证书，也不解密或中间人拦截 HTTPS。
